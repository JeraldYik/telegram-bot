  ...
  bot.command('food', async (ctx) => {
    ...
    if (Object.keys(latlng).length > 0) {
      ...
      bot.on('callback_query', async (ctx) => {
        ...
        while (pages <= 3) {
          const nearbySearchURI = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?${
            nextPageToken === ''
              ? `location=${latlngFromFile.lat},${latlngFromFile.lng}&radius=${radius}&minprice=${minPrice}&maxprice=${maxPrice}&opennow=&type=${type}`
        ...
        if (results.length > 0) {
          ...
          ctx.reply('Searched address did not return any results. Please try again with another input or reformat your input.', {
            parse_mode: 'HTML'
          });
        }
      });
